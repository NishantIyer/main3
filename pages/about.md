---
title: About
description: About Me
image: https://cdn.discordapp.com/attachments/971299427715272734/1114144376311001168/Nishant_Iyer.png
plum: false
---

Having exhibited innate precociousness and an insatiable curiosity from a young age, I cultivated an avid interest in all things, technology and computers. By age 7, I was actively designing and building  [robots](https://drive.google.com/drive/folders/1idyy4DyDHI7uGDj3E00iVGRDthxPJJ2b?usp=drive_link) to compete in academic science fairs, cementing my lifelong passion for computer science. This early exposure sparked a fascination with computers and programming. I cultivated discipline through martial arts, voraciously consuming online materials. Soon, I was coding full-fledged applications and automating everyday tasks. My enthusiasm never waned; I programmed for the joy of creation all the way to higher secondary.
As a driven, detail-oriented, and solutions-focused individual, my core competencies lie in architecting, designing, and developing robust, scalable [cloud native solutions](https://nishantiyer.netlify.app/projects#cloud) leveraging AWS, Alibaba Cloud, and other prominent cloud providers. With production-level experience across the breadth of the AWS and Alibaba Cloud ecosystems, I adeptly tackle complex challenges in resource allocation, horizontal scaling, container orchestration, service mesh implementation, database optimization, while engineering fault-tolerant distributed ledger frameworks. I have also authored numerous cloud-based open source solutions including modules, plugins, and marketplace images for pentesters. In addition, I have fostered successful multi-cloud integration initiatives. My recent work includes developing CoaaS, a serverless and cost-effective cloud infrastructure tailored for personal use cases and startups. In the virtualization arena, I have architected high-performance VDIs for financial infrastructures, managed large-scale KVM hypervisor clusters, and pioneered the development of a novel remote desktop architecture for DVPC. I remain actively engaged with the Ethereum, Cardano, and other blockchain developer communities, contributing extensively to core open source infrastructure projects.

My systems thinking ability, cross-disciplinary perspective, and steady focus on fundamentals have enabled me to gain working knowledge across web stacks, embedded programming, blockchain platforms and more. I live code - it is oxygen to me. With Linux and Unix being second nature after years of being an avid power user and active member of the Arch Linux community, I feel equally comfortable building performance-sensitive, low-level systems programs as well as scripting. While not an expert in computer networks, I have a strong working knowledge of core concepts.

In addition, I have experience in bot development leveraging transformer models, creating custom Discord and Telegram bots focused on GPT fine-tuning, coherence, and prompt engineering. As a hobbyist web developer, I particularly excel at back-end development, having built countless websites and web apps. I consider myself highly proficient in Python, moderately experienced in C and Java, and a true lover of Haskell.

My diverse engineering skills are supplemented with business acumen obtained from running my own startup. I'm figuring out product-market fit, presenting to investors, going it alone, and gaining traction. This combination allows me to be perspicacious in enterprise settings. I'm currently working part-time on Portena, my idea and startup, which is the leading remote desktop architecture for everyday users. Although it is still in its early phases, it takes significant focus and dedication, which makes me less active in the sector at the moment. I hope to re-engage more completely after Portena obtains traction.
My unparalleled work ethic is a hallmark, manifested in my relentless dedication and stoic perseverance, with weeks stretching into 110-hour(18hr/day) work marathons during holidays and as I'm still a high school goer, I work 6-7 hours after reaching back home and work throughout the classes as well. I somehow manage my academics well and in order to further develop it and make it to the market, I need to put in at least 16-17 hours everyday for work. This is one of the main reasons why my product (portena) is not in the market yet despite being theortically completely feasible , but yet I grind through this stage hoping for a healthier schedule.

In my precious little downtime, I enjoy pondering deep philosophical thoughts, analyzing stocks and markets, practicing calisthenics and hitting the gym often, playing the piano, and pursuing various other intellectual and creative stimuli to keep my mind engaged. 

In short, I pride myself on being a consummate **Jack of all trades, master of none, yet excelling in my role as a work-thirsty jack.**

## Experience 

- **Open-Source Developer** - As an open source developer, I have had the opportunity to contribute to various modules, frameworks, and Linux distributions that are relied upon by people around the world. Through my involvement in online communities such as Discord and Slack, I have connected with other developers, shared my projects, and collaborated on open source projects, gaining valuable experience in software development and making meaningful contributions to the open source software community.
- **Ethereum** - I was a moderator and part of API development team building core infrastructure for over 1 year.
- **Cardano Community** - Developed Plutus frameworks, monitored HPC markets, made marketplace images for community and ensured HAs and FTs for DLT (Decentralized Ledger Technologies) for over 1 year.

## Indulgence in Universities and Certification

- MIT Disruptive Technology: Blockchain: More details about this course can be found at [MIT Disruptive Technology: Blockchain](https://www.mit.edu)
- Princeton Computer Architecture: Additional information about this course can be accessed at [Princeton Computer Architecture](https://www.princeton.edu)
- (ISC)2 - Networks and Communications Security
- University of California - The Blockchain: More information about this course can be accessed at [University of California - The Blockchain](https://www.uc.edu)
- University of Irvine - Arduino Platform: More information about this course can be accessed at [University of Irvine - Arduino Platform](https://www.irvine.edu)

## Skills
- HCL
- Stocks and Forex (occassional investor, hustler's university)
- Market Analysis (during my tenure in cardano community, hustler's univ.)
- Technical Presentation (articulate and presented and won in CBSE National Science Fair, Academic Events, Pitching product to many pre-seed funding companies and many more)
- Haskell Development
- Blockchain Development (Backend source management, api , smartcotract, ledgers)
- Network Protocol Specialization (built APRA)
- Application Security (Built privacy respecting frontends and backends to popular privacy disrepecting media platforms like Nix and Beatville)
- Deep Understanding of AWS and Alibaba Cloud (Took courses in AWS Academy and managed ledgers in Alibaba CLoud platforms)
- Resource Allocation
- Bot Development - NLP, Transformers (Built 5-6 popular discord bots, custom nueral network, fine-tuned GPT and GPT-based applications like Proxie)
- Network Management (Neso Academy and working knowledge of Wireshark, Snort, ESP32, Nmap and other networking tools)
- Remote Connection Protocols (Specialization)
- Smart Contracts (Built upon during my tenure in Ethereum Communtiy)
- Robotic Development (Using Arduino)
- Web Development (As a hobby)
- Linux/Unix Systems (Arch Activist, Power-user, current - fedora , built own distro)
- Python (second nature to me)
- Expertise in building Fault Tolerant Systems (Ledgers)
- Worked on CaaS for Orchestration and VM Deployment (Self-empolyed community DLT engineer, worked across various deployment platforms)
- Virtual Machine Optimization (Intelligent OS methods for improving performance)
- Bandwidth Optimization (APRA)
- Architecture of high-performance VDIs (Online forums, financial-crypto use cases, hpc market analyzing and weekly reporting in slack)
- Distributed Computing (MiT 6.824 spring batch)
- Application architecture (Built many apps)
- Computational creativity (Developed my own computational model - DVPC which is a remote compute model that optimizes resources in a way that has never been done.)
- Serverless infrastructure design (e.g., CoaaS)
- Hybrid Cloud (AWS, Alibaba)
- Amazon EC2 (Worked closely with while prototyping Portena)
- Open Systems Architecture 
- HPC - Ledgers
- Open Source (12,024 contributions in the last year)
- Community Moderation (handling disrespectful people, intrusion attempts in online discord/slack servers)
- Brand Building (Personal and Community)

## Certifications

- Applied Ethical Hacking and Rules of Engagement: [Certificate](https://media.licdn.com/dms/image/C562DAQH0OYppOEfgrA/profile-treasury-image-shrink_800_800/0/1676173344683?e=1686844800&v=beta&t=pQrdkXJbVbAMMnDJdPK71pWo08-bg7lgurWuKMxZnf0)
- Certified ScrumMaster (CSM): [Certificate](https://www.credential.net/3d137be0-ec6a-42b9-82ef-667d9c72212a#gs.qkn3lv)
- Linux Professional Institute: [Certification](https://www.lpi.org/our-certifications/lpic-3-305-overview)
- Ethical Hacking: [Certificate](https://media.discordapp.net/attachments/932975399552577576/933004999196278814/The_Complete_Cyber_Security_Course_Hackers_Exposed.png?width=574&height=406)
- End Point Protection: [Certificate](https://media.discordapp.net/attachments/932975399552577576/933005243556433960/End_Point_Protection.png?width=574&height=406)
- Cisco NDG Linux Essentials: [Certificate](https://id.cisco.com/signin/refresh-auth-state/00pKZ9NRC7-T8QIpP-cUNFQiwv2s6yTr41XFBioDFD)

## Entrepreneurial Venture

Portena stands at the forefront of remote computing innovation, offering a paradigm-shifting solution that addresses the pressing challenges faced by users today. Drawing inspiration from my experiences in working with Cardano, I conceived this breakthrough idea and embarked on a relentless journey to bring it to life. With advanced technologies like the uNet remote connection protocol and DRATP, we have crafted a seamless and user-centric experience that surpasses traditional PCs in terms of stability, latency, and graphics streaming.
The wastage of computing resources becomes a thing of the past with Portena. Our intelligent resource allocation mechanisms ensure optimal utilization, eliminating energy drain and unnecessary costs. We have democratized access to powerful computing capabilities by providing a cost-effective alternative, breaking down the barriers that hindered remote desktop adoption for personal users.
The simplicity of the Portena experience is remarkable. Minimal client-side requirements allow users to effortlessly connect, stream, and edit their remote desktop activities. This is made possible by our globally dispersed CoaaS infrastructures, providing fast and reliable access from any corner of the world.
Join us as we redefine the boundaries of remote computing. Together, we will shape a future where Portena becomes the catalyst for enhanced productivity and connectivity, transforming the way we work, compute, and thrive...

![Portena Image](https://media.discordapp.net/attachments/971299427715272734/1117070920721371289/sirg.png?width=395&height=491)

Portena is still in its nascent ideation stages, and I have been ardently engrossed in prototyping, rigorous testing, and extensive research. For further insights into the remarkable technologies it embodies, please look at the blog section on our website.

If you are interested and eager to learn more about Portena or wish to get in touch, kindly reach out to me at [nishant.iyer62@gmail.com](mailto:nishant.iyer62@gmail.com). I would be delighted to engage in a profound discussion about this visionary project.


## Papers

- Virtualized Distributed Elastic Compute: [Paper](https://example.com)
- Proxie (NLP): [Technical Documentation](https://example.com)
- More: [Blank]


.... and i don't mean it metaphorically or rhetorically or poetically or theoretically or in any other fancy way - i grind the f off the ide - straight f.
